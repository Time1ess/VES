...
public class MenuIcon : MonoBehaviour {
	...
	void UpdateShader()
	{
		GetComponent<Renderer>().material.color = (objectSelectable ? Color.white : Color.gray);
	}
	...
	void ChangeIconShader()
	{
		...
		GetComponent<Renderer>().material.SetFloat ("_Transparency", standardTransparency + factor);
        GetComponent<Renderer>().material.SetFloat("_FilledTransparency", standardTransparency + factor + filledTransparency);
		GetComponent<Renderer>().material.SetFloat ("_PercentageFilled", percentage);
